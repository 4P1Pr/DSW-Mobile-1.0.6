...
class AspirationPageActivity : BaseActivity<AspirationPageContract.Presenter>(),
    AspirationPageContract.View, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener,
        OnMapReadyCallback{

    override val presenter by inject<AspirationPageContract.Presenter>()
    ...
    override fun setAttachedPicture(imageUri: Uri?) {
        binding.apply {
            Glide.with(this@AspirationPageActivity)
                    .load(imageUri)
...
//                val to = "pengaduan@depok.go.id"
//                val subject = spinnerComplaintpageCategoryspinner.selectedItem.toString()
//                val messageemail = edittextComplaintpageMessage.getText().toString()
//                //val uri = Uri.parse("file://" + presenter.onPressAttachFile())
//                //val imagecrop = imageviewComplaintpageAttachedimage.getDrawingCache().to(attachFile())
//
...
//                    //intent.putExtra(Intent.EXTRA_STREAM, imagecrop)
...
//                    return@setOnClickListener startActivity(Intent.createChooser(intent, "choose"))
            ...
            fm.beginTransaction().replace(R.id.view_complaintpage_mapcontainer, supportMapFragment)
                    .commit()
            supportMapFragment.getMapAsync(this@AspirationPageActivity)
        }
